...
class PlanXmlManager(XmlManager):
    """
    ...
    def initialize_xml_elements(self):
        self._component_xml_tag = xml_tags.CO_SIM_XML_PLAN_ROOT_TAG
    ...
    def __build_items_to_be_arranged_dict_from_whole_dict(self):
        ...
        try:
            items_to_be_arranged_dict = self._main_xml_sections_dicts_dict[xml_tags.CO_SIM_XML_PLAN_ARRANGEMENT]
        except KeyError:
            self._logger.error('{} has no <{}>...</{}> section'.format(self._xml_filename,
                                                                       xml_tags.CO_SIM_XML_PLAN_ARRANGEMENT,
                                                                       xml_tags.CO_SIM_XML_PLAN_ARRANGEMENT))
        ...
        for key, value in sorted(items_to_be_arranged_dict.items()):
            ...
            if arrangement_duty not in constants.CO_SIM_ARRANGEMENT_DUTIES_TUPLE:
                self._logger.error(
                    '{} has <{}><{}> a wrong value {}'.format(self._xml_filename,
                                                              key,
                                                              xml_tags.CO_SIM_XML_ARRANGEMENT_DUTY,
                                                              arrangement_duty))
    ...
    def __build_action_plan_dict_from_whole_dict(self):
        ...
        try:
            xml_action_plan_dict = self._main_xml_sections_dicts_dict[xml_tags.CO_SIM_XML_PLAN_ACTION_PLAN]
        except KeyError:
            self._logger.error('{} has no <{}}>...</{}}> section'.format(self._xml_filename,
                                                                         xml_tags.CO_SIM_XML_PLAN_ACTION_PLAN,
                                                                         xml_tags.CO_SIM_XML_PLAN_ACTION_PLAN))
            return enums.XmlManagerReturnCodes.XML_TAG_ERROR
    ...
    def build_particular_sections_dicts(self):
        ...
        if not items_to_be_arranged_return == enums.XmlManagerReturnCodes.XML_OK:
        ...
        if not action_plan_builder_return == enums.XmlManagerReturnCodes.XML_OK:
